...
| `src/features/spoilers/interactions/spoiler-buttons.js` | Anti-spoiler transformation and modal handling for normal messages. | interaction-handler | spoilers | `src/features/spoilers/interactions/spoiler-buttons.js` | keep | Canonical spoiler interaction and rendering helper module. |
| `handlers/spoiler-button-interactions.js` | Legacy adapter for spoiler interaction module. | adapter | spoilers | `src/features/spoilers/interactions/spoiler-buttons.js` | done-adapter | Preserves old handler require path. |
...
| `src/features/twitter/interactions/media-pagination.js` | Twitter media pagination and image merge/split controls. | interaction-handler | twitter | `src/features/twitter/interactions/media-pagination.js` | keep | Canonical Twitter media pagination interaction. |
| `handlers/twitter-pagination-interactions.js` | Legacy adapter for Twitter media pagination. | adapter | twitter | `src/features/twitter/interactions/media-pagination.js` | done-adapter | Preserves old handler path. |
...
| `src/features/twitter/interactions/translation.js` | Classic Twitter translation button flow, cache/state, embed update. | interaction-handler | twitter/translation | `src/features/twitter/interactions/translation.js` | keep | Canonical classic Twitter translation interaction. |
| `handlers/twitter-translate-interactions.js` | Legacy adapter for classic Twitter translation button flow. | adapter | twitter/translation | `src/features/twitter/interactions/translation.js` | done-adapter | Preserves old handler path. |
...
| `src/features/twitter/interactions/v2-router.js` | Twitter V2 translate/original, expand/collapse, reload, spoiler modal flow router. | interaction-handler | twitter | `src/features/twitter/interactions/v2-router.js` | keep | V2 action logic is split under `src/features/twitter/interactions/v2/`. |
| `handlers/twitter-v2-interactions.js` | Legacy adapter for Twitter V2 interaction router. | adapter | twitter | `src/features/twitter/interactions/v2-router.js` | done-adapter | Preserves old handler path. |
...
| `src/core/config/config-loader.js` | Runtime config loader/accessor for JSON config files. | service | core/config | `src/core/config/config-loader.js` | keep | Centralizes config paths and reloads. |
| `src/core/config/pekoembed-config.json` | PekoEmbed feature config. | config | core/config | `src/core/config/pekoembed-config.json` | keep | Moved from `tfd-system/config/pekoembed-config.json`; old JSON path intentionally removed. |
| `src/core/config/supported-sites.json` | Supported site registry. | config | core/config | `src/core/config/supported-sites.json` | keep | Moved from `tfd-system/config/supported-sites.json`; can become extractor registry metadata later. |
| `src/core/config/tfd-config.json` | Runtime config data for HTTP/timeouts/rendering. | config | core/config | `src/core/config/tfd-config.json` | keep | Runtime reads go through `src/core/config/config-loader.js`; old JSON path intentionally removed. |
...
- `src/features/spoilers/interactions/spoiler-buttons.js` (`handlers/spoiler-button-interactions.js` adapter)
