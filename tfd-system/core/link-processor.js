...
class LinkProcessor {
    ...
    async processMessage(message) {
        ...
        try {
            ...
            if (!this.checkRateLimit(message.author.id)) {
                console.log(`[Ermiana-LinkProcessor] 用戶 ${message.author.id} 觸發速率限制`);
                return [];
            }
            ...
            for (const url of urls) {
                try {
                    ...
                    console.error(`[Ermiana-LinkProcessor] 處理 URL 失敗: ${url} - ${error.message}`);
                    // 繼續處理其他 URL
                }
            }
            ...
            console.error(`[Ermiana-LinkProcessor] 處理訊息失敗: ${error.message}`);
            return [];
        }
    }
    ...
    async processSingleURL(url, message) {
        ...
        if (this.processedCache.has(url)) {
            ...
            if (Date.now() - cached.timestamp < 300000) { // 5分鐘快取
                ...
                if (!cached.result || (cached.result && cached.result.success === false)) {
                    console.log(`[Ermiana-LinkProcessor] 清除無效快取: ${url}`);
                    this.processedCache.delete(url);
                } else {
                    console.log(`[Ermiana-LinkProcessor] 使用快取結果: ${url}`);
        ...
        if (!this.isSiteEnabled(matchResult.siteName)) {
            console.log(`[Ermiana-LinkProcessor] 網站已停用: ${matchResult.siteName}`);
            return null;
        }
        ...
        try {
            ...
            console.error(`[Ermiana-LinkProcessor] 提取失敗: ${matchResult.siteName} - ${error.message}`);
            return null;
        }
    }
    ...
    clearCache() {
        ...
        console.log('[Ermiana-LinkProcessor] 快取已清空');
    }
    ...
    reloadConfig() {
        try {
            ...
            console.log('[Ermiana-LinkProcessor] 配置已重新載入');
            return true;
        } catch (error) {
            console.error(`[Ermiana-LinkProcessor] 重新載入配置失敗: ${error.message}`);
            return false;
        }
    }
