...
const { loadTfdConfig, reloadTfdConfig } = require('../config/config-loader');
...
class TFDSystem {
    ...
    async initialize() {
        try {
            ...
            tfd.sys('TFD', 'TFD 系統初始化完成');

            return true;
        } catch (error) {
            tfd.sysError('TFD', `❌ 初始化失敗: ${error.message}`);
            return false;
        }
    }
    ...
    _getMessageHandler() {
        if (!this.messageHandler) {
            const MessageHandler = require('../../../tfd-system/core/message-handler-v2');
    ...
    async processMessage(message) {
        ...
        try {
            ...
            this.stats.errors++;
            tfd.sysError('TFD', `處理訊息失敗: ${error.message}`);
            return [];
        }
    }
    ...
    async processMessageUpdate(oldMessage, newMessage) {
        ...
        try {
            ...
            this.stats.errors++;
            tfd.sysError('TFD', `處理訊息更新失敗: ${error.message}`);
            return [];
        }
    }
    ...
    validateConfig() {
        ...
        for (const field of requiredFields) {
            if (!(field in this.config)) {
                tfd.sysError('TFD', `配置缺少必要欄位: ${field}`);
                return false;
            }
        }
    ...
    reloadConfig() {
        try {
            ...
            tfd.sysError('TFD', `重新載入配置失敗: ${error.message}`);
            return false;
        }
    }
     ...
     */
    async shutdown() {
        tfd.sys('TFD', '🔄 正在關閉 TFD 系統...');

        this.clearCache();
        this.initialized = false;

        tfd.sys('TFD', '✅ TFD 系統已關閉');
    }
}
