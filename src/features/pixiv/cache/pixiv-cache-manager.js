...
class PixivCacheManager {
    ...
    /**
     * Delete a single artwork cache file.
     * @param {string} artworkId - Artwork ID
     * @returns {Promise<boolean>} true when deleted, false when no cache file existed
     */
    async deleteArtworkCache(artworkId) {
        if (!artworkId) return false;

        const cacheFile = this.getArtworkCacheFile(artworkId);
        try {
            await fs.unlink(cacheFile);
            tfd.sys('PixivCache', `Deleted artwork cache: ${artworkId}.json`);
            return true;
        } catch (error) {
            if (error.code === 'ENOENT') {
                tfd.sys('PixivCache', `Artwork cache did not exist: ${artworkId}.json`);
                return false;
            }

            tfd.sysError('PixivCache', `Delete artwork cache failed: ${error.message}`);
            throw error;
        }
    }
